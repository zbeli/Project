...
void calculate_query(struct query_info *temp_q,  struct file_info* info, struct thrd_pool *thread_pool, fstats *stats){
	...
	for(i = 0; i < num_pred; i++){
		...
		if(pred.flag == -1){
			...
			res = RadixHashJoin(&rel_R, &rel_S, thread_pool);
            ...
            update_results_filter(result_lists, &tmp_list1, pred.tuple_1.rel, info, temp_q, current_pred, thread_pool);
			free_result(&tmp_list1);
            tmp_list1.start_list=NULL;
            tmp_list1.list_size=0;
            tmp_list1.counter=0;	
    ...
    print_sum(result_lists, temp_q, info, thread_pool); /*Calculate sum*/
...
void print_sum(struct result* result, struct query_info *query, struct file_info *info, struct thrd_pool *pool){
	...
	// parallel calculation of sum
/*	int jobs = (int)pow(2,h1);		
...
thrd_pool_wake(pool);
	for (int i = 0; i < query -> cols_count; ++i){
		...
		rel[i] = query->rels[query->cols_to_print[i].rel];
		col[i] = query->cols_to_print[i].col;
		res[i] = &result[query->cols_to_print[i].rel];
		...
		job_queue_push(pool, &Sum, args[i]);
	}
thrd_pool_wait(pool);
